...
export default function AudioQueuePanel(props: AudioQueueProps) {
  ...
  const activeQueueId = useAudioPlayer((state) => state.current?.queueId);
        ...
        <List fontSize="sm">
          ...
          {queue.map((audio, index) => (
            <ListItem
              ...
              paddingY={2}
              onClick={() => {
                playIndex !== index && setPlayIndex(index);
              }}
            ...
            >
              ...
              <Flex flex="1" justifyContent="flex-end" width="50px">
                <CloseButton
                  isDisabled={activeQueueId === audio.queueId}
                  onClick={async (e) => {
                    e.stopPropagation();
                    await removeIndexFromQueue(queueContext, index);
                  }}
                />
              </Flex>
            </ListItem>
