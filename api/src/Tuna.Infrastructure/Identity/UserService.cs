...
public class UserService : IIdentityService
{
    ...
    public async Task<NewIdentityUserResult> CreateUserAsync(string userName, string email, string password,
        CancellationToken cancellationToken = default)
    {
        ...
        if (!result.Succeeded) return (NewIdentityUserResult)IdentityResult.Failed(result.Errors.ToArray());

        return NewIdentityUserResult.Success(user.Id);
    }

    public async Task<IdentityResult> UpdateUserNameAsync(string identityId, string userName,
        CancellationToken cancellationToken = default)
    {
    ...
    public async Task<IdentityResult> UpdateEmailAsync(string identityId, string email,
        CancellationToken cancellationToken = default)
    {
        var user = await _userManager.FindByIdAsync(identityId);
    ...
    public async Task<IdentityResult> UpdatePasswordAsync(string identityId, string currentPassword, string newPassword,
        CancellationToken cancellationToken = default)
    {
        var user = await _userManager.FindByIdAsync(identityId);
